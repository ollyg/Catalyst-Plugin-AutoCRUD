package SQL::Translator::Producer::CustomHTML;
...
sub produce {
    ...
    for my $table ($schema->get_tables) {
        ...
        if (exists $table->extra->{_relationships}
                and ref $table->extra->{_relationships} eq ref {}
                and scalar keys %{$table->extra->{_relationships}}) {
            push @html, $q->b("Relationships:"), $q->br;
            push @html, $q->ul(
                map { $q->li(
        ...
        for my $field ( @fields ) {
            ...
            my $extra = join ',<br>', map {
                (ref $field->extra->{$_} eq ref '')
                    ? ("$_ = ". $field->extra->{$_})
                    : ("$_ = ". ref $field->extra->{$_})
            } sort {$a cmp $b} grep {$_ !~ m/^_/} keys %{$field->extra};
            ...
            push @html,
                $q->Tr(
                    ...
                    $q->td({ -class => "FieldCellOther" }, join(',<BR>', @other)),
                    $q->td({ -class => "FieldCellFK" }, $fk),
                    $q->td({ -class => "FieldCellExtra" }, $extra),
                );
        }
        push @html, $q->end_table;
